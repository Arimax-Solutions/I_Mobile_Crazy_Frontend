...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDailyCosts = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/dailyCost/today');
        const data = await response.json();

        if (response.ok) {
          setDailyCosts(data.data); // Assuming the response structure is as you've shown
        } else {
          console.log(data.message || 'Failed to fetch daily costs');
        }
      } catch (err) {
        console.log('Error fetching daily costs');
      } finally {
        console.log(false);
      }
    };
...
// Header
          ...
          pdf.text(`Date: ${new Date().toLocaleDateString()}`, 205, 35, { align: 'right' });


// "I MOBILE CRAZY" Text
          pdf.setFontSize(18);
          pdf.setTextColor(255, 255, 255);
          pdf.text("I MOBILE CRAZY", headerMarginLeft + 60, headerMarginTop + 10, {
            align: "center",
          });
...
// Additional Information
          pdf.setFontSize(10);
          pdf.setTextColor(255, 255, 255);
          pdf.text("Distributors of Mobile", headerMarginLeft + 55, headerMarginTop + 15, {
            align: "center",
          });
          pdf.text("Phones & Accessories", headerMarginLeft + 55, headerMarginTop + 20, {
            align: "center",
          });
          ...
          pdf.text(`Daily Income: ${totalIncome.toFixed(2)}`, 205, headerMarginTop + 35, { align: 'right' });
          pdf.text(`Daily Cost: ${dailyCost.toFixed(2)}`, 205, headerMarginTop + 40, { align: 'right' });
          pdf.text(`Total Income: ${netIncome.toFixed(2)}`, 205, headerMarginTop + 45, { align: 'right' });
        };
                  ...
                  <div className='bg-white p-8 rounded shadow-lg w-full max-w-full max-h-full z-60'>
                    ...
                    <div id='pdf-content'>
                      <table className="max-w-8l w-full mx-auto bg-white">
                        <thead>
                        <tr>
                          <th className="py-6 px-8 border bg-green-800 text-white text-xl">OrderId</th>
                          <th className="py-6 px-8 border bg-green-800 text-white text-xl">Brand</th>
                          <th className="py-6 px-8 border bg-green-800 text-white text-xl">Name</th>
                          <th className="py-6 px-8 border bg-green-800 text-white text-xl">Customer/Shop Name</th>
                          <th className="py-6 px-8 border bg-green-800 text-white text-xl">Date</th>
                          <th className="py-6 px-8 border bg-green-800 text-white text-xl">Total</th>
                        </tr>
                        </thead>
                        <tbody>
                            ...
                            <>
                              ...
                              {order.items?.map((item: any) => (
                                  <tr key={`${order.retail_order_id || order.wholesale_order_id || order.return_order_id}-item-${item.item_id}`}>
                                    <td className="py-6 px-8 border">{order.retail_order_id || order.wholesale_order_id || order.return_order_id}</td>
                                    <td className="py-6 px-8 border">{item.brand}</td>
                                    <td className="py-6 px-8 border">{item.name}</td>
                                    <td className="py-6 px-8 border">{order.customer ? order.customer.name : order.shop.shop_name}</td>
                                    <td className="py-6 px-8 border">{new Date(order.date).toLocaleDateString()}</td>
                                    <td className="py-6 px-8 border">{order.total_amount}</td>
                                  </tr>
                              ))}
                              ...
                              {order.imeis?.map((imei: any) => (
                                  <tr key={`${order.retail_order_id || order.wholesale_order_id || order.return_order_id}-imei-${imei.id}`}>
                                    <td className="py-6 px-8 border">{order.retail_order_id || order.wholesale_order_id || order.return_order_id}</td>
                                    <td className="py-6 px-8 border">{imei.modelId?.name || 'N/A'}</td>
                                    <td className="py-6 px-8 border">{imei.imei}</td>
                                    <td className="py-6 px-8 border">{order.customer ? order.customer.name : order.shop.shop_name}</td>
                                    <td className="py-6 px-8 border">{new Date(order.date).toLocaleDateString()}</td>
                                    <td className="py-6 px-8 border">{order.total_amount}</td>
                                  </tr>
                      ...
                      {/* Daily Cost Heading */}
                      <h2 className="text-2xl font-bold text-center mb-4">Daily Cost</h2>

                      <table className="max-w-6xl w-full mx-auto bg-white border">
                        <thead>
                        <tr>
                          <th className="py-4 px-6 border bg-green-800 text-white">Date</th>
                          <th className="py-4 px-6 border bg-green-800 text-white">Amount</th>
                          <th className="py-4 px-6 border bg-green-800 text-white">Reason</th>
                        </tr>
                        </thead>
                        <tbody>
                        {dailyCosts.map((cost) => (
                            <tr key={cost.daily_cost_id}>
                              <td className="py-4 px-6 border">{new Date(cost.date).toLocaleDateString()}</td>
                              <td className="py-4 px-6 border">{cost.amount.toFixed(2)}</td>

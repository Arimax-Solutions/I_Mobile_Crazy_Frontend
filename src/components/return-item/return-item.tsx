...
export default function ReturnItem() {
  ...
  const [selectedReturnPhone, setSelectedReturnPhone] =
    useState<ReturnPhones | null>(null);
  ...
  const validateFields = () => {
    const newErrors = {
      ...
      contact_number:
        ...
        (contact_number.trim() === "" || contact_number.length !== 10)
  ...
  const handleItemUpdateOnClick = async () => {
    ...
    try {
      const response = await axios.put(
        `${backend_url}/api/returnItem/${selectedReturnPhone.return_phone_id}`,
        updatedPhoneData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.status === 200) {
        ...
        setReturnPhones((prevItems) =>
          prevItems.map((item) =>
            item.return_phone_id === selectedReturnPhone.return_phone_id
              ? updatedPhoneData
              : item
          )
        );
          ...
          <tbody className="divide-y divide-gray-200">
            {returnPhones
              ...
              .map((user) => (
                ...
                >
                  <td className="px-6 py-2 whitespace-nowrap transition-colors duration-300 hover:text-black hover:font-bold">
                    {user.return_phone_id}
                  </td>
                  <td className="px-6 py-2 whitespace-nowrap transition-colors duration-300 hover:text-black hover:font-bold">
                    {user.brand}
                  </td>
